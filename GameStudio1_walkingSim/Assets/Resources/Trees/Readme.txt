~*~*~*~*~ TREE MAKING INSTRUCTIONS (from Ricardo) ~*~*~*~*~

    I have created a "Test Tree" prefab that you can use ase a baseline for creating many trees.

    To create your own tree:

    1. Drag the "Test Tree" prefab from the Resources/Trees folder into your scene.
    2. Click on the "Test Tree" object in the scene and go to GameObject > Break Prefab Instance (at the top of Unity)
    3. This "Test Tree" has a certain number of settings under its "Procedural Tree" component. These settings control how the tree is generated by the code.
    4. Click "Rand Seed" to generate a slightly different tree using the current settings. Click "Rand Tree" to generate a completely new tree using random settings.
    5. Each time you want to make a new tree in the scene, just duplicate an already existing tree object, move the duplicate somewhere else, and click "Rand Seed"
    6. If you like a certain set of settings and want to save them, you can just turn your current tree into a prefab by dragging it into the Resources/Trees folder
